...
describe(AssetService.name, () => {
  ...
  describe('transferAssets', () => {
    it('should reject transferring to yourself', async () => {
      const auth = AuthFactory.create();
      await expect(
        sut.transferAssets(auth, { ids: ['asset-1'], newOwnerId: auth.user.id }),
      ).rejects.toBeInstanceOf(BadRequestException);
    });
    ...
    it('should require asset write access', async () => {
      ...
      await expect(sut.transferAssets(auth, { ids: ['asset-1'], newOwnerId })).rejects.toBeInstanceOf(
        BadRequestException,
      );
    });
    ...
    it('should transfer ownership when new owner has enabled receiving transfers', async () => {
      ...
      await sut.transferAssets(auth, { ids: ['asset-1', 'asset-2'], newOwnerId });
